...
(define-http-type (:string)
    :lookup-assertion (match restrictions
			((list :min min)
			 `(assert-http (>= (length ,parameter) ,min)))
			((list :max max)
			 `(assert-http (>= ,max (length ,parameter))))
			((list :min min :max max)
			 `(assert-http (>= ,max (length ,parameter) ,min)))))

(define-http-type (:integer)
    :type-expression `(parse-integer ,parameter :junk-allowed t)
    :lookup-assertion (match restrictions
			((list :min min)
			 `(assert-http (>= ,parameter ,min)))
			((list :max max)
			 `(assert-http (>= ,max ,parameter)))
			((list :min min :max max)
			 `(assert-http (>= ,max ,parameter ,min)))))
  ...
  (loop with res = body
     ...
     do (match arg
	  ((list* arg-sym type restrictions)
	   (let ((arg-exp `(cdr (assoc ,(->keyword arg-sym) parameters))))
	     (setf res
		   `(let ((,arg-sym ,(or (type-expression arg-exp type restrictions) arg-exp)))
		      ,@(awhen (lookup-assertion arg-sym type restrictions) `(,it))
		      ,res))))
...
(defmacro define-json-handler ((name) (&rest args) &body body)
  `(bind-handler ,name (make-closing-handler (:content-type "application/json") ,args (json:encode-json-to-string ,@body))))
